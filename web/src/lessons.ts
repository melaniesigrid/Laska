 ...
 * `TutorialBoard` wrapper can highlight squares and gate input to the expected
 * move(s). No runtime renderer exists yet — see the note at the bottom of this
 * file and the report for what the hookup requires.
 *
...
// NOTE — runtime hookup is NOT wired yet. This module is the validated lesson
// DATA + builder only. To surface these in the app you still need (1) a
// `TutorialBoard` wrapper over `BoardView` that highlights `step.highlight`,
// gates input to `step.expectedMoves`, auto-plays `actor: 'opponent'` steps, and
// shows prompt/hint/successText; (2) a lessons page (copy ReplayPage.tsx) added
// to App.tsx's `view` union; (3) `localStorage` progress. See the handoff report.
