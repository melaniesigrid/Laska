...
function stateFromPosition(
  position: string,
  variantId: VariantId = "laska",
): GameState {
...
export function useOnline() {
  ...
  const [rematchOfferBy, setRematchOfferBy] = useState<PlayerColor | null>(
    null,
  );
  ...
  const [spectateLastMove, setSpectateLastMove] = useState<MoveDTO | null>(
    null,
  );
      ...
      switch (msg.type) {
          ...
          setUser((u) =>
            u
              ? {
                  ...u,
                  rating: msg.rating,
                  ratingDeviation: msg.ratingDeviation,
                  rank: msg.rank,
                }
              : u,
          );
        ...
        case "match.end": {
          ...
          if (user && msg.ratingChange) {
            const mine =
              match?.myColor === "W"
                ? msg.ratingChange.white
                : msg.ratingChange.black;
            setUser((u) =>
              u ? { ...u, rating: mine.after, rank: mine.rank.after } : u,
            );
          }
          break;
        }
        ...
        case "spectate.started": {
          ...
          setSpectateState(
            stateFromPosition(msg.state.position, msg.state.variant),
          );
  ...
  const displaySpectateClock = useMemo<ClockDTO | null>(() => {
    ...
    return {
      whiteMs:
        spectateClock.running === "W"
          ? Math.max(0, spectateClock.whiteMs - elapsed)
          : spectateClock.whiteMs,
      blackMs:
        spectateClock.running === "B"
          ? Math.max(0, spectateClock.blackMs - elapsed)
          : spectateClock.blackMs,
  ...
  const acceptDraw = useCallback(() => {
    if (match)
      client.send({ type: "match.acceptDraw", matchId: match.matchId });
  }, [client, match]);
  ...
  const sendEmote = useCallback(
    (emote: EmoteId) => {
      if (match)
        client.send({ type: "match.emote", matchId: match.matchId, emote });
    },
    [client, match],
  );
  ...
  const sendTyping = useCallback(
    (typing: boolean) => {
      if (match)
        client.send({ type: "match.typing", matchId: match.matchId, typing });
    },
    [client, match],
  );
  ...
  const declineRematch = useCallback(() => {
    if (match)
      client.send({ type: "match.rematchDecline", matchId: match.matchId });
    setRematchSent(false);
    setRematchOfferBy(null);
  }, [client, match]);
  const declineDraw = useCallback(() => {
    if (match)
      client.send({ type: "match.declineDraw", matchId: match.matchId });
  }, [client, match]);
